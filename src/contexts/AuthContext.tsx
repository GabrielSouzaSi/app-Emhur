...
import { applyTeamHeader } from "@/server/applyServerContext"
import {
	storageAuthTokenGet,
...
export function AuthContextProvider({ children }: AuthContextProviderProps) {
	...
	const primaryTeamId = useMemo(() => getPrimaryTeamId(user), [user])
	const primaryTeamAlias = useMemo(() => getPrimaryTeamAlias(user), [user])
	...
	async function signIn(email: string, password: string) {
		...
		try {
			...
			if (data?.user && data?.access_token) {
				...
				applyTeamHeader(getPrimaryTeamId(data.user))
				setUser(data.user)
			}
	...
	async function loadUserData() {
		...
		try {
			...
			if (userLogged && token) {
				...
				applyTeamHeader(getPrimaryTeamId(userLogged))
				setUser(userLogged)
			} else {
				applyAuthHeader(null)
				applyTeamHeader(null)
	...
	const value = useMemo(
		...
		[user, roles, teams, primaryTeamId, primaryTeamAlias, can, isBootstrapping, authSubmitting],
	)
