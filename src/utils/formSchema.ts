...
import { isFieldVisible } from "./formVisibility"

export function buildVisibleSectionRows(section: FormSection, values: Record<string, any>) {
	...
	const remaining = visibleFields.filter((f) => !usedNames.has(f.name))
